...
async def home():
    return HTML.replace("__LOGO_DARK__", LOGO_DARK).replace("__LOGO_PLAY_DARK__", LOGO_PLAY_DARK).replace("__LOGO_WATERMARK__", LOGO_WATERMARK)
...
<head>
  ...
  <style>
    ...
    .playPill:hover{ border-color: rgba(107,228,255,.35); }
    .playPill img{
    ...
    .playPill .playTxt{
      font-weight:900;
      letter-spacing:.2px;
      background: linear-gradient(90deg, #f6b14a 0%, #ff6aa7 45%, #b36cff 100%);
...
<body class="theme-cyber">
  ...
  <div class="app" id="app" style="display:none;">
    <div class="topbar">
      ...
      <a class="playPill" href="/play/" target="_blank" rel="noopener noreferrer" title="HestioPlay">
        <img src="__LOGO_PLAY_DARK__" alt="HestioPlay"/>
        <span class="playTxt">HestioPlay</span>
      </a>
...
# Mount static assets (used by HestioPlay puzzles), if present
try:
...
# -----------------------------
# PlayRoom / session state
# -----------------------------

@dataclass
class PlayPlayerConn:
    id: str
    name: str
    ws: WebSocket
...
class PlayRoom:
    ...
    players: Dict[str, PlayPlayerConn] = field(default_factory=dict)
    game: Optional[PlayGameSession] = None
...
class PlayRoomManager:
    def __init__(self) -> None:
        ...
        self.rooms_by_id: Dict[str, PlayRoom] = {}
        self.pool: Dict[Tuple[int, str], List[str]] = {}

    async def assign_room(self, target_players: int, game_type: str) -> PlayRoom:
        async with self.lock:
            ...
            rid = make_room_id()
            room = PlayRoom(id=rid, target_players=target_players, game_type=game_type)
            self.rooms_by_id[rid] = room
            ids.append(rid)
            return room

    async def remove_room_from_pool(self, room: PlayRoom) -> None:
        key = (room.target_players, room.game_type)
        ids = self.pool.get(key, [])
        if room.id in ids:
            ids.remove(room.id)

    async def mark_room_not_joinable(self, room: PlayRoom) -> None:
        async with self.lock:
            await self.remove_room_from_pool(room)

    async def mark_room_joinable(self, room: PlayRoom) -> None:
        async with self.lock:
    ...
    async def maybe_cleanup_room(self, room: PlayRoom) -> None:
        async with self.lock:
...
@app.get("/play/public-rooms")
async def public_rooms() -> JSONResponse:
    rooms = await play_manager.list_public_rooms()
...
async def play_ws_send(ws: WebSocket, msg_type: str, payload: Dict[str, Any]) -> None:
    await ws.send_text(json.dumps({"type": msg_type, "payload": payload}))


async def room_broadcast(room: PlayRoom, msg_type: str, payload: Dict[str, Any]) -> None:
    dead: List[str] = []
    for pid, p in room.players.items():
        try:
            await play_ws_send(p.ws, msg_type, payload)
        except Exception:
            dead.append(pid)
    for pid in dead:
...
def room_state_payload(room: PlayRoom) -> Dict[str, Any]:
    return {
        "roomId": room.id,
        "targetPlayers": room.target_players,
        "gameType": room.game_type,
...
async def start_countdown_if_ready(room: PlayRoom) -> None:
    ...
    await play_manager.mark_room_not_joinable(room)
...
async def start_game(room: PlayRoom) -> None:
    ...
    gt = room.game_type
    session = PlayGameSession(game_id=game_id, game_type=gt, seed=seed, started_at_ms=started, ends_at_ms=ends, finished=False)
...
async def finish_game(room: PlayRoom, reason: str) -> None:
    game = room.game
    ...
    room.status = "waiting"
    room.game = None
    await play_manager.mark_room_joinable(room)
    await room_broadcast(room, "room:state", room_state_payload(room))
    await start_countdown_if_ready(room)


async def handle_submit(room: PlayRoom, player_id: str, payload: Dict[str, Any]) -> None:
    game = room.game
...
async def ws_endpoint(ws: WebSocket) -> None:
    ...
    current_room: Optional[PlayRoom] = None

    try:
        await play_ws_send(ws, "session:hello", {"playerId": player_id, "name": player_name})
        ...
        while True:
            ...
            if msg_type == "training:join":
                ...
                if current_room is not None:
                    ...
                    await play_manager.mark_room_joinable(old)
                    await play_manager.maybe_cleanup_room(old)
                    current_room = None

                room = await play_manager.assign_room(target, game_type)
                current_room = room
                room.players[player_id] = PlayPlayerConn(id=player_id, name=player_name, ws=ws)
                ...
                if current_room is not None:
                    ...
                    await room_broadcast(room, "room:state", room_state_payload(room))
                    await play_manager.mark_room_joinable(room)
                    await play_manager.maybe_cleanup_room(room)
                    current_room = None
                await play_ws_send(ws, "room:left", {})
                ...
                await play_ws_send(ws, "pong", {"ts": now_ms()})

            else:
                await play_ws_send(ws, "error", {"message": f"Unknown message type: {msg_type}"})
