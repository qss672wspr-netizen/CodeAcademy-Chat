...
<head>
  ...
  <style>
    ...
    .dot{ width:10px; height:10px; border-radius:999px; background:#ff3b3b;
      box-shadow:0 0 14px rgba(255,59,59,.22); }
    .dot.ok{ background:#20c997; box-shadow:0 0 14px rgba(32,201,151,.22); }
    ...
    /* Center chat panel more transparent (to show watermark) */
    #centerPanel{
      background: rgba(0,0,0,0.18) !important;
    }
    body.theme-light #centerPanel{
...
.sidehead{
    ...
    body.theme-light .logoutbtn{ background:rgba(11,18,32,.04); }
...
<script>
  ...
  function doLogout(){
    try{
      stopReconnect();
      if(ws){
        try{ ws.onopen = ws.onmessage = ws.onclose = null; }catch{}
        try{ ws.close(); }catch{}
      }
    }catch{}
    ws = null;
